...
class PiperPanel(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Label(outer, text="关节", style="Joint.TLabel").grid(row=8, column=0, padx=3)
        ttk.Label(outer, text="当前角度 (°)", style="Joint.TLabel").grid(row=8, column=1, padx=3)
        ttk.Label(outer, text="目标角度 (°)", style="Joint.TLabel").grid(row=8, column=2, columnspan=2, padx=3)
        ttk.Label(outer, text="相对变化 (°)", style="Joint.TLabel").grid(row=8, column=4, padx=3)
        ttk.Label(outer, text="应用", style="Joint.TLabel").grid(row=8, column=5, padx=3)
        for index, name in enumerate(JOINT_NAMES):
            row = index + 9
            outer.rowconfigure(row, minsize=21)
            ttk.Label(outer, text=name, style="Joint.TLabel").grid(row=row, column=0, padx=3, pady=0)
            label = ttk.Label(outer, text="--", width=10, style="Joint.TLabel")
            label.grid(row=row, column=1, padx=3, pady=0)
            self.current_labels.append(label)
            low, high = JOINT_LIMITS[index]
            slider = ttk.Scale(outer, from_=low, to=high, length=230,
                               command=lambda value, i=index: self.slider_changed(i, value))
            slider.grid(row=row, column=2, padx=(3, 5), pady=0)
            self.sliders.append(slider)
            ttk.Label(outer, textvariable=self.targets[index], width=8, style="Joint.TLabel").grid(row=row, column=3, padx=(0, 3), pady=0)
            ttk.Entry(outer, textvariable=self.relative_inputs[index], width=7, style="Joint.TEntry").grid(row=row, column=4, padx=3, pady=0)
            ttk.Button(outer, text="应用", style="Joint.TButton", command=lambda i=index: self.apply_relative(i)).grid(row=row, column=5, padx=3, pady=0)
        ttk.Separator(outer).grid(row=15, column=0, columnspan=6, sticky="ew", pady=5)
        ttk.Checkbutton(outer, text="我已确认机械臂周围无人、无障碍物，且可随时按下实体急停", variable=self.safety_ok).grid(row=16, column=0, columnspan=6, sticky="w")
        ttk.Button(outer, text="使能电机（不会运动）", command=self.enable_motors).grid(row=17, column=0, columnspan=6, sticky="ew", pady=(4, 2))
        ttk.Button(outer, text="恢复初始姿态（零位，当前速度）", command=self.move_home).grid(row=18, column=0, columnspan=6, sticky="ew", pady=2)
        ttk.Button(outer, text="发送低速目标（当前速度）", command=self.send_target).grid(row=19, column=0, columnspan=6, sticky="ew")
        ttk.Label(outer, text="连续运动每次 +/- 为 XYZ 10 mm、姿态 1°；滑块松开时执行。速度由上方“应用速度”设置。", foreground="#555555").grid(row=20, column=0, columnspan=6, pady=(4, 0), sticky="w")
