...
class TracerRos:
    ...
    def __init__(self, on_pose_update) -> None:
        ...
        self.remote_command_age = float("inf")
        self.remote_command_seen = 0.0
        ...
        self.rclpy = None
        self.bridge = None
        self._ssh_commands: queue.Queue[tuple[float, float]] = queue.Queue(maxsize=1)
        self.use_ssh_bridge = os.environ.get("TRACER5_USE_SSH_BRIDGE", "0") == "1"
        # Bridge generation lets restarted bridges ignore error reports from
        # the threads of the bridge they replaced.
        self._bridge_generation = 0
        self._restart_lock = threading.Lock()
        self._last_bridge_restart = 0.0
        self._watchdog_thread: threading.Thread | None = None

        try:
            if self.use_ssh_bridge:
                self._start_ssh_bridge()
                self._watchdog_thread = threading.Thread(target=self._bridge_watchdog, daemon=True)
    ...
    def _start_ssh_bridge(self) -> None:
        ...
        self._last_bridge_restart = time.monotonic()
        # ServerAliveInterval makes a silently dead Wi-Fi/Ethernet link kill
        # ssh within ~15 s instead of leaving the sender blocked on write().
        bridge = subprocess.Popen(
            ["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=10",
             "-o", "ServerAliveInterval=5", "-o", "ServerAliveCountMax=3",
             f"skki@{host}", f"bash -lc {shlex.quote(remote)}"],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            text=True, bufsize=1,
        )
        self.bridge = bridge
        self.thread = threading.Thread(target=self._spin_ssh, args=(bridge, generation), daemon=True)
        self.thread.start()
        self.sender_thread = threading.Thread(target=self._send_bridge_commands, args=(bridge, generation), daemon=True)
        self.sender_thread.start()
        self.stderr_thread = threading.Thread(target=self._read_bridge_errors, args=(bridge, generation), daemon=True)
        self.stderr_thread.start()

    def restart_ssh_bridge(self) -> bool:
        """Reconnect a stalled command stream without cancelling its target.

        Rate limited so the plan loop and the watchdog cannot tear down a
        freshly started bridge.  Returns True when a restart really happened.
        """
        if not self.use_ssh_bridge:
            return False
        with self._restart_lock:
            if time.monotonic() - self._last_bridge_restart < 5.0:
                return False
            self._last_bridge_restart = time.monotonic()
            # Mute the old bridge's threads before terminating it, otherwise
            # their dying write() can overwrite this restart's clean state.
            self._bridge_generation += 1
            old = self.bridge
    ...
    def _bridge_watchdog(self) -> None:
        """Keep an SSH control bridge alive in every mode, not only plans.

        A dead ssh process or an exited sender thread used to require closing
        the whole panel; now the bridge is rebuilt automatically.
        """
        while self._running:
            time.sleep(1.0)
            if not self.use_ssh_bridge or not self._running:
                continue
            bridge = self.bridge
            if bridge is None:
                continue
            sender_alive = self.sender_thread is not None and self.sender_thread.is_alive()
            if bridge.poll() is None and sender_alive:
                continue
            self.restart_ssh_bridge()

    def _send_bridge_commands(self, bridge, generation) -> None:
        """The SSH pipe may stall; never let it hold the control-state lock."""
        while self._running and bridge.poll() is None:
            try:
                linear, angular = self._ssh_commands.get(timeout=0.2)
            except queue.Empty:
                continue
            if not bridge.stdin:
                return
            try:
                bridge.stdin.write(json.dumps({"type": "twist", "linear": linear, "angular": angular}) + "\n")
                bridge.stdin.flush()
            except (BrokenPipeError, OSError):
                if self._bridge_generation == generation:
                    self.error = "远程 SSH 控制通道已断开"
                return

    def _queue_bridge_command_locked(self, linear: float, angular: float) -> None:
        """Retain only the latest command; putting must never block control."""
        try:
            while True:
                self._ssh_commands.get_nowait()
        except queue.Empty:
            pass
        try:
            self._ssh_commands.put_nowait((float(linear), float(angular)))
        except queue.Full:
            pass

    def _read_bridge_errors(self, bridge, generation) -> None:
        if not bridge.stderr:
            return
        for line in bridge.stderr:
            text = line.strip()
            if text and self._bridge_generation == generation:
                self.error = f"远程控制通道：{text[-180:]}"

    def _spin_ssh(self, bridge, generation) -> None:
        if not bridge.stdout:
            return
        # Reader and controller are separated so an idle remote pose stream
        # never delays a stop command from the UI.
        reader = threading.Thread(target=self._read_bridge_poses, args=(bridge,), daemon=True)
        reader.start()
        ...
        if self._running and self._bridge_generation == generation and not self.error:
            self.error = "远程 SSH 控制通道已关闭"

    def _read_bridge_poses(self, bridge) -> None:
        if not bridge.stdout:
            return
        for line in bridge.stdout:
            ...
            with self._lock:
                ...
                if "cmd_linear" in event and "cmd_angular" in event:
                    self.remote_command = (float(event["cmd_linear"]), float(event["cmd_angular"]))
                    self.remote_command_age = float(event.get("cmd_age", float("inf")))
                    self.remote_command_seen = time.monotonic()
    ...
    def _target_command_locked(self) -> tuple[float, float]:
        ...
        # The Tracer odometry is quantised at roughly centimetre scale.  A
        # sub-centimetre target tolerance leaves the controller issuing its
        # minimum command forever when the reported pose stalls 1 cm short.
        # Keep a user-requested looser tolerance, but never require a more
        # precise final reading than the chassis can report.
        arrival_position_tol = max(self.position_tolerance_m, 0.015)
        arrival_yaw_tol = max(self.yaw_tolerance_rad, math.radians(1.5))
        if self.target_drive_straight:
            # Pure relative X commands mean "drive along the current vehicle
            # heading".  Do not steer for small IMU/odometry side drift,
            # otherwise a nominally straight 10 cm move visibly oscillates.
            # A large heading change is different: continuing to command
            # forward velocity can make the chassis drive away from its
            # world-frame target (even backwards in the displayed frame).
            remaining = math.cos(self.target.yaw) * dx + math.sin(self.target.yaw) * dy
            heading_error = wrap_angle(self.target.yaw - self.pose.yaw)
            if abs(remaining) <= arrival_position_tol:
            ...
            if abs(heading_error) > math.radians(12.0):
                self.target_phase = "recover_heading"
        ...
        if distance <= arrival_position_tol:
        ...
        if self.target_phase == "final_turn" and distance > arrival_position_tol:
            self.target_phase = "turn_to_path"
            return self._target_command_locked()
        if abs(yaw_error) <= arrival_yaw_tol:
            self.auto_state, self.target_name, self.target_drive_straight, self.target_phase = "idle", "", False, "idle"
            return 0.0, 0.0
    ...
    def _publish_locked(self, linear: float, angular: float) -> None:
        if self.use_ssh_bridge:
            if not self.bridge or self.bridge.poll() is not None or not self.bridge.stdin:
                return
            self._queue_bridge_command_locked(linear, angular)
        else:
    ...
    def control_snapshot(self):
        """Small diagnostic snapshot for the combined automatic-plan UI."""
        with self._lock:
            return (self.auto_state, self.target_phase, self._last_command,
                    self.remote_command, self.remote_command_age, self.remote_command_seen)
