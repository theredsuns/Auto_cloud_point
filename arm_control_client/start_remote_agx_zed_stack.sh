...
START_ZED_WRAPPER="${START_ZED_WRAPPER:-0}"
if [[ "$START_ZED_WRAPPER" == "1" ]]; then
    if pgrep -f '[z]ed_wrapper.*zed_camera.launch.py' >/dev/null; then
        ...
        echo "[2/4] Starting full ZED wrapper (explicitly requested)..."
    ...
    echo "[2/4] Skipping full ZED wrapper. The control panel starts the lightweight RGB/depth capture path when needed."
fi
