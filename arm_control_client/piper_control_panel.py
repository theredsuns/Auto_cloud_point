...
class PiperPanel(tk.Tk):
    def __init__(self, preview=False):
        ...
        self.current = [None] * 6
        # Keep chassis control and odometry on the vehicle computer.  The
        # wired SSH bridge avoids fragile cross-machine DDS discovery while
        # the ZED/point-cloud work continues locally on this computer.
        os.environ.setdefault("TRACER5_USE_SSH_BRIDGE", "1")
        ...
        self.auto_open_3d = tk.BooleanVar(value=bool(saved_settings.get("auto_open_3d", False)))
        ...
        self.cloud_popup = None
        self.cloud_preview_queue: queue.Queue[tuple] = queue.Queue()
        ...
        self.vehicle_motion_info = tk.StringVar(value="小车执行：待命")
        self.auto_capture_end_info = tk.StringVar(value="规划结果：待命")
        self._auto_capture_end_reason_text = "待命"
        self.capture_countdown_info = tk.StringVar(value="抓拍计时：未开始")
        self.fusion_status_info = tk.StringVar(value="点云融合：待命")
        ...
        self._auto_open_3d_active = bool(self.auto_open_3d.get())
        ...
        self._build()
        self.after(80, self._drain_cloud_preview_queue)
    ...
    def _build_camera_panel(self, parent):
        ...
        ttk.Label(panel, text="右侧可设置抓拍时自动启动 3D", foreground="#666666").grid(row=5, column=1, sticky="e")
    ...
    def _set_auto_open_3d(self):
        """Snapshot the 3D display choice before capture workers start."""
        self._auto_open_3d_active = bool(self.auto_open_3d.get())
    ...
    def _save_capture_detection_settings(self):
        try:
            ...
            settings.update({"no_target_wait_s": self._no_target_wait_s_active,
                             ...
                             "auto_open_3d": self._auto_open_3d_active})
    ...
    def _set_vehicle_motion_detail(self, text):
        """Detailed vehicle diagnostics, safe to call from plan workers."""
        self.after(0, lambda value=str(text): self.vehicle_motion_info.set(f"小车执行：{value}"))

    def _set_auto_capture_end_reason(self, text):
        """Keep the final reason visible after the general status refreshes."""
        self._auto_capture_end_reason_text = str(text)
        self.after(0, lambda value=str(text): self.auto_capture_end_info.set(f"规划结果：{value}"))
    ...
    def _ensure_capture_scan(self):
        ...
        self.fusion_completed = 0
        self.fusion_failed = 0
        self.after(0, lambda: self.fusion_status_info.set("点云融合：新会话待命（已完成 0 张）"))
        return scan
    ...
    def start_new_capture_scan(self):
        ...
        self.fusion_completed = 0
        self.fusion_failed = 0
        self.fusion_status_info.set("点云融合：新会话待命（已完成 0 张）")
    ...
    def _cloud_preview_worker(self, cloud_path, show_popup=False):
        try:
            ...
            # Tk widgets must only be touched by the main thread.  A direct
            # self.after() from this Open3D worker can remain blocked on Tcl
            # in some Linux/Tk builds, leaving the UI at “正在加载”.
            self.cloud_preview_queue.put(("preview", points, colors, cloud_path.name, show_popup))
        except Exception as error:
            self.cloud_preview_queue.put(("error", str(error)))

    def _drain_cloud_preview_queue(self):
        """Apply Open3D results on Tk's main thread and reliably show 3D."""
        if not self.winfo_exists():
            return
        try:
            while True:
                item = self.cloud_preview_queue.get_nowait()
                if item[0] == "preview":
                    _, points, colors, name, show_popup = item
                    self._apply_cloud_preview(points, colors, name, show_popup)
                else:
                    self.fusion_status_info.set(f"点云融合：3D 预览读取失败：{item[1][-120:]}")
        except queue.Empty:
            pass
        self.after(80, self._drain_cloud_preview_queue)
    ...
    def _apply_cloud_preview(self, points, colors, name, show_popup=False):
        ...
        self.fusion_status_info.set(
            f"点云融合：已完成 {self.fusion_completed} 张，3D 已更新（{len(points):,} 个预览点）"
        )
        if show_popup:
    ...
    def _show_fused_cloud_window(self, scan):
        """Let the opt-in Open3D viewer refresh itself from the fused PLY."""
        fused = Path(scan) / "fusion" / "fused_icp.ply"
        if not fused.is_file():
            self.fusion_status_info.set("点云融合：未找到 fused_icp.ply，3D 窗口未更新")
        elif self._auto_open_3d_active:
            self._start_auto_cloud_viewer(scan, wait_for_first_fusion=True)
    ...
    def display_latest_fused_cloud(self):
        """Open the current session's fused cloud, loading it from disk if needed."""
        if self.routine_running or self.capture_busy or self.fusion_pending:
            self.fusion_status_info.set(
                "点云融合：抓拍或 ICP 仍在进行；结束后再点击显示 3D，避免图形窗口冲突"
            )
            return
        if self.last_cloud_points is not None:
            if self.capture_scan is not None:
                self._start_auto_cloud_viewer(self.capture_scan)
            else:
                self.open_cloud_popup()
            return
        fused = self.capture_scan / "fusion" / "fused_icp.ply" if self.capture_scan else None
        if fused is not None and fused.is_file():
            self.fusion_status_info.set("点云融合：正在打开独立 Open3D 视图…")
            self._start_auto_cloud_viewer(self.capture_scan)
            return
        messagebox.showinfo("尚无点云", "请先完成至少一次抓拍和点云融合。")
        ...
        try:
            result = subprocess.run(
                ["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=10", REMOTE_HOST,
                 "bash ~/zed_code/scripts/start_direct_zed_tcp_bridge.sh"],
    ...
    def _build(self):
        ...
        ttk.Label(vehicle_summary, textvariable=self.vehicle_motion_info, foreground="#3f5f78", wraplength=210).pack(anchor="w", fill="x", pady=(3, 0))
        ...
        ttk.Label(vehicle_summary, textvariable=self.auto_capture_end_info, foreground="#9b2c2c", wraplength=210).pack(anchor="w", fill="x", pady=(3, 0))
        ttk.Separator(vehicle_summary).pack(fill="x", pady=8)
        ttk.Label(vehicle_summary, text="点云融合状态", font=("Sans", 9, "bold")).pack(anchor="w")
        ttk.Label(vehicle_summary, textvariable=self.fusion_status_info, foreground="#5b3b8c", wraplength=210).pack(anchor="w", fill="x", pady=(3, 0))
        ttk.Checkbutton(
            vehicle_summary, text="抓拍时自动启动 3D 点云窗口", variable=self.auto_open_3d,
            command=self._set_auto_open_3d,
        ...
        ttk.Checkbutton(settings, text="安全确认：机械臂周围无人、无障碍物，可随时急停", variable=self.safety_ok).grid(
            row=2, column=0, columnspan=6, sticky="w", pady=(5, 0)
        )
        ttk.Button(settings, text="机械臂回到初始位置", command=self.move_home).grid(
            row=2, column=6, columnspan=3, sticky="ew", padx=(8, 0), pady=(5, 0)
        )
    ...
    def _generate_auto_capture_plan(self):
        ...
        for index, record in enumerate(self.vehicle_motion_records, 1):
            pieces = self._split_vehicle_record(record)
            for suffix, piece in pieces:
                label = f"小车点 {index}" if len(pieces) == 1 else f"小车点 {index}（{suffix}）"
                plan.append({"kind": "vehicle", "name": label, "record": piece})
    ...
    @staticmethod
    def _split_vehicle_record(record):
        """Separate a combined XY+Yaw instruction into safe serial steps."""
        clean = {"type": str(record["type"]), "x": float(record["x"]),
                 "y": float(record["y"]), "yaw": float(record["yaw"])}
        has_translation = abs(clean["x"]) > 1e-6 or abs(clean["y"]) > 1e-6
        has_turn = abs(clean["yaw"]) > 1e-6
        if has_translation and has_turn:
            return [
                ("移动", {**clean, "yaw": 0.0}),
                ("转向", {**clean, "x": 0.0, "y": 0.0}),
            ]
        return [("", clean)]

    @classmethod
    def _copy_auto_capture_plan(cls, plan):
        copied = []
        for step in plan:
            ...
            if step.get("kind") == "camera":
                ...
                if isinstance(record, dict) and all(key in record for key in ("type", "x", "y", "yaw")):
                    pieces = cls._split_vehicle_record(record)
                    base_name = str(step.get("name", "小车点"))
                    for suffix, piece in pieces:
                        name = base_name if len(pieces) == 1 else f"{base_name}（{suffix}）"
                        copied.append({"kind": "vehicle", "name": name, "record": piece})
        return copied
    ...
    def _run_auto_capture_plan_worker(self, plan, vehicle_speed):
        ...
        try:
            ...
            if result.returncode or not self._wait_for_home_joint_pose(timeout_s=70.0):
                ...
                return
            completed = True
            self.after(0, lambda: self.status.set(f"自动抓拍规划完成：已抓拍 {captures} 帧；后台点云融合将继续并逐帧显示 3D。"))
        except Exception as error:
            ...
            elif self.auto_capture_plan_cancelled:
                self._set_auto_capture_phase("规划已取消")
                self._set_auto_capture_end_reason("用户取消")
            else:
                self._set_auto_capture_phase("规划已停止：请查看上方状态")
                if self._auto_capture_end_reason_text == "运行中":
                    self._set_auto_capture_end_reason("某一步未完成或通信未确认")
            self._set_vehicle_motion_detail("当前没有运行中的小车规划步骤")
    ...
    def _run_auto_capture_vehicle_step(self, record, index, total, linear, angular):
        ...
        requested_yaw = float(record["yaw"])
        has_translation = abs(x) > 1e-6 or abs(y) > 1e-6
        # Old saved/current plans can still contain a combined X/Y + Yaw
        # record.  Execute its translation only; yaw must be an explicit,
        # separate record so a completed move can never wait on a turn.
        yaw_for_this_step = 0.0 if has_translation and abs(requested_yaw) > 1e-6 else requested_yaw
        deferred_turn = has_translation and abs(requested_yaw) > 1e-6
        if record["type"] == "相对":
            target = Pose2D(pose.x + math.cos(pose.yaw) * x - math.sin(pose.yaw) * y,
                            pose.y + math.sin(pose.yaw) * x + math.cos(pose.yaw) * y,
                            wrap_angle(pose.yaw + math.radians(yaw_for_this_step)))
            straight = abs(y) < 1e-6 and abs(yaw_for_this_step) < 1e-6
        else:
            target = Pose2D(origin.x + math.cos(origin.yaw) * x - math.sin(origin.yaw) * y,
                            origin.y + math.sin(origin.yaw) * x + math.cos(origin.yaw) * y,
                            wrap_angle(origin.yaw + math.radians(yaw_for_this_step)))
            straight = False
        self._set_auto_capture_phase(f"小车运动：规划点 {index}/{total}")
        mode = "仅移动" if deferred_turn else ("移动" if has_translation else "仅转向")
        suffix = f"；原记录 Yaw={requested_yaw:.1f}° 已留给独立转向步骤" if deferred_turn else ""
        self._set_vehicle_motion_detail(
            f"步骤 {index}/{total}（{mode}），记录 X={x * 100:.1f}，Y={y * 100:.1f} cm，Yaw={yaw_for_this_step:.1f}°{suffix}"
        )
        ok, reason = self.tracer.start_target(target, f"规划小车点 {index}/{total}", linear, angular, straight=straight)
        if not ok:
            self._set_auto_capture_end_reason(f"步骤 {index}/{total}：小车目标未启动：{reason}")
        ...
        deadline = started + max(30.0, 8.0 + math.hypot(x, y) / max(linear, 0.02) * 4.0)
        last_progress_at = started
        best_error = float("inf")
        last_ui_update = 0.0
        bridge_reconnected = False
        unconfirmed_since = None
        reconnect_started = 0.0
        ...
        while time.monotonic() < deadline:
            ...
            if reconnect_deadline is not None:
                # The terminated bridge stops delivering poses while the new
                # SSH session connects (several seconds), so suspend the
                # normal 1 s localization timeout and wait for the rebuilt
                # bridge to report again.  A receipt stamped more than 1 s
                # after the restart must come from the new bridge, not from
                # buffered stragglers of the old one.
                if newest > reconnect_started + 1.0 and now - newest <= 1.0:
                    reconnect_deadline = None
                    started = now
                    deadline = max(deadline, now + 15.0)
                    last_progress_at = now
                    last_ui_update = 0.0
                    unconfirmed_since = None
                    self._set_auto_capture_phase(f"小车控制通道已恢复：重试步骤 {index}/{total}")
                elif now >= reconnect_deadline:
                    self.tracer.stop_all()
                    self.vehicle_route_running = False
                    self._set_auto_capture_end_reason(
                        f"步骤 {index}/{total}：重建 SSH 控制桥后仍未收到小车定位"
                    )
                    self._set_auto_capture_phase("小车控制通道重建失败：已停止")
                    self._set_vehicle_motion_detail(
                        f"步骤 {index}/{total}：重连 {now - reconnect_started:.0f}s 后仍无定位数据，已停止"
                    )
                    self.after(0, lambda: self.status.set(
                        "自动抓拍规划：小车控制通道重建失败，已停止；请检查小车端网络或负载。"
                    ))
                    return False
                else:
                    if now - last_ui_update >= 0.5:
                        ...
                        self._set_auto_capture_phase(
                            f"正在重建小车 SSH 控制通道（{now - reconnect_started:.0f}s）…"
                        )
                    time.sleep(0.1)
                    continue
            if time.monotonic() - newest > 1.0:
                self.tracer.stop_all()
                self._set_auto_capture_end_reason(f"步骤 {index}/{total}：小车定位超时")
                self.after(0, lambda: self.status.set("自动抓拍规划：小车定位超时，已停止。"))
                return False
            if state == "idle":
                self.vehicle_route_running = False
                self._set_auto_capture_phase("小车已到位：继续下一步骤")
                self._set_vehicle_motion_detail(f"步骤 {index}/{total} 已到位，开始下一环节")
                return True
            if current_pose is not None:
                position_error = math.hypot(target.x - current_pose.x, target.y - current_pose.y)
                signed_yaw_error = wrap_angle(target.yaw - current_pose.yaw)
                yaw_error = abs(signed_yaw_error)
                # A weighted error also works for an in-place rotation.
                error = position_error + 0.20 * yaw_error
                if error < best_error - 0.004:
                    best_error, last_progress_at = error, time.monotonic()
            now = time.monotonic()
            if now - last_ui_update >= 0.5:
                elapsed = now - started
                control_state, control_phase, command, remote_command, remote_age, remote_seen = self.tracer.control_snapshot()
                self._set_auto_capture_phase(
                    f"小车运动：规划步骤 {index}/{total}（到位 {elapsed:.1f}s）"
                )
                if current_pose is not None:
                    self._set_vehicle_motion_detail(
                        f"步骤 {index}/{total}｜{control_phase}｜本机 v={command[0]:.2f}，ω={command[1]:.2f}；"
                        f"小车端 v={remote_command[0]:.2f}，ω={remote_command[1]:.2f}\n"
                        f"当前位置 X={current_pose.x * 100:.1f}，Y={current_pose.y * 100:.1f} cm；"
                        f"距目标 {position_error * 100:.1f} cm，当前/目标 Yaw "
                        f"{math.degrees(current_pose.yaw):.1f}°/{math.degrees(target.yaw):.1f}°，"
                        f"角差 {math.degrees(signed_yaw_error):+.1f}°\n"
                        f"里程计进展等待 {now - last_progress_at:.1f}/8.0 s"
                    )
                last_ui_update = now
            if self.tracer.use_ssh_bridge and now - started > 1.5:
                _state, _phase, command, remote_command, remote_age, remote_seen = self.tracer.control_snapshot()
                wants_motion = abs(command[0]) > 0.01 or abs(command[1]) > 0.01
                # A brief receipt lag is normal when the vehicle computer is
                # busy streaming ZED frames, or when local ICP/open3d load
                # starves the panel's own threads; only a mismatch sustained
                # for a full second means the command stream is really wedged.
                remote_matches = (abs(remote_command[0] - command[0]) < 0.03 and
                                  abs(remote_command[1] - command[1]) < 0.08 and
                                  remote_age < 1.0 and now - remote_seen < 2.0)
                if wants_motion and not remote_matches:
                    if unconfirmed_since is None:
                        unconfirmed_since = now
                    if now - unconfirmed_since >= 1.0:
                        if not bridge_reconnected:
                            bridge_reconnected = True
                            self._set_auto_capture_phase("小车 SSH 命令未确认：正在自动重连并重试")
                            self._set_vehicle_motion_detail(
                                f"步骤 {index}/{total}：正在重建小车控制通道，随后重试当前目标"
                            )
                            self.tracer.restart_ssh_bridge()
                            reconnect_started = time.monotonic()
                            reconnect_deadline = reconnect_started + 18.0
                            deadline = max(deadline, reconnect_deadline + 5.0)
                            unconfirmed_since = None
                            continue
                        self.tracer.stop_all()
                        self.vehicle_route_running = False
                        self._set_auto_capture_end_reason(
                            f"步骤 {index}/{total}：SSH 命令未到达底盘（远端 v={remote_command[0]:.2f}）"
                        )
                        self._set_auto_capture_phase("小车命令未到达底盘：已停止")
                        self._set_vehicle_motion_detail(
                            f"步骤 {index}/{total}：本机 v={command[0]:.2f}、ω={command[1]:.2f}，"
                            f"小车端确认 v={remote_command[0]:.2f}、ω={remote_command[1]:.2f}"
                        )
                        self.after(0, lambda: self.status.set(
                            "自动抓拍规划：小车持续未确认运动命令，已停止；请检查小车端负载或网络后重试。"
                        ))
                        return False
                else:
                    unconfirmed_since = None
            if now - last_progress_at >= 8.0:
                self.tracer.stop_all()
                self.vehicle_route_running = False
                self._set_auto_capture_end_reason(f"步骤 {index}/{total}：8 秒无里程计进展")
                self.after(0, lambda: self.status.set(
                    "自动抓拍规划：小车 8 秒无里程计进展，已停止。请检查急停、CAN 和底盘使能。"
                ))
                self._set_auto_capture_phase("小车无运动反馈：已停止")
                if current_pose is not None:
                    self._set_vehicle_motion_detail(
                        f"步骤 {index}/{total}：8 秒无进展，已停止；剩余 {position_error * 100:.1f} cm，"
                        f"角差 {math.degrees(yaw_error):.1f}°"
                    )
                else:
                    self._set_vehicle_motion_detail(f"步骤 {index}/{total}：8 秒无位置/角度进展，已发送停止命令")
                return False
            time.sleep(0.1)
        self.tracer.stop_all()
        self.vehicle_route_running = False
        self._set_auto_capture_end_reason(f"步骤 {index}/{total}：到位超时")
        self._set_vehicle_motion_detail(f"步骤 {index}/{total}：到位超时，已发送停止命令")
    ...
    def _run_vehicle_records(self):
        ...
        self.vehicle_route_running = True
        self.vehicle_route_cancelled = False
        records = [piece for item in self.vehicle_motion_records
                   for _suffix, piece in self._split_vehicle_record(item)]
    ...
    def _run_vehicle_auto_capture_worker(self, records, arm_records, vehicle_speed):
        ...
        try:
            ...
            self.after(0, lambda n=capture_index: self.status.set(
                f"小车-机械臂联动抓拍完成：已保存 {n} 帧；后台点云融合将继续并逐帧显示 3D。"
            ))
        except Exception as error:
            self.auto_capture_failed = True
            ...
            if not self.auto_capture_failed:
                self._close_auto_capture_views()
            if completed:
                self._set_auto_capture_phase("联动抓拍已完成：后台融合可继续")
            elif self.auto_capture_failed:
                self._set_auto_capture_phase("联动抓拍异常结束：请查看上方状态")
            elif self.vehicle_route_cancelled:
                self._set_auto_capture_phase("联动抓拍已取消")
            else:
                self._set_auto_capture_phase("联动抓拍已停止：请查看上方状态")
            self._set_vehicle_motion_detail("当前没有运行中的小车规划步骤")
            self._set_capture_countdown("抓拍计时", 0.0, self._capture_settle_s_active)
            self.after(0, self.refresh_status)
    ...
    def _load_control_settings():
        ...
        try:
            ...
            return {
                ...
                "auto_open_3d": bool(data.get("auto_open_3d", False)),
            }
        except (OSError, ValueError, TypeError, json.JSONDecodeError):
            return {}
    ...
    def _prepare_auto_capture(self):
        ...
        scan = self._ensure_capture_scan()
        if self._auto_open_3d_active:
            self.after(0, lambda value=scan: self._start_auto_cloud_viewer(value, wait_for_first_fusion=True))
        deadline = time.monotonic() + 25.0
        while self.routine_running and time.monotonic() < deadline:
            rgb, depth = self.remote_camera.live_frame()
            # The lightweight /zed_scanner source streams RGB continuously,
            # while its depth is produced for an actual capture request.  Do
            # not reject that valid mode during preview preparation; the
            # first _capture_auto_frame still requires RGB + depth strictly.
            if rgb is not None:
                ...
                return True
            time.sleep(0.5)
        self.after(0, lambda: self.status.set("远程 ZED RGB 画面未在 25 秒内就绪，已停止。"))
        return False

    def _start_auto_cloud_viewer(self, scan, wait_for_first_fusion=False):
        """Open one independent 3D viewer; it can wait for the first PLY."""
        viewer = ONLINE_SCAN_ROOT / "live_fusion_viewer.py"
        fused = Path(scan) / "fusion" / "fused_icp.ply"
        if (not viewer.is_file() or (not wait_for_first_fusion and not fused.is_file()) or
        ...
        try:
            ...
            self.status.set("已启动独立 3D 点云窗口：等待首帧融合完成后自动显示并刷新。")
        except OSError as error:
    ...
    def _enqueue_fusion(self, scan, stamp, sequence_index):
        """Start serial background ICP without blocking the capture/motion plan."""
        ...
        self.after(0, lambda i=sequence_index, count=pending: self.fusion_status_info.set(
            f"点云融合：已完成 {self.fusion_completed} 张；第 {i} 帧已入队，待处理 {count} 帧"
        ))
        self.after(0, lambda i=sequence_index, count=pending: self.status.set(
            f"位置 {i}：图片/掩码已保存，ICP 在后台融合（{count} 帧待处理）；抓拍和运动继续执行。"
        ))
    ...
    def _fusion_worker_loop(self):
        while True:
            ...
            self.after(0, lambda i=sequence_index: self.fusion_status_info.set(
                f"点云融合：已完成 {self.fusion_completed} 张，正在 ICP 融合第 {i} 帧"
            ))
            result = subprocess.run(
            ...
            if result.returncode:
                ...
                self.after(0, lambda i=sequence_index: self.fusion_status_info.set(
                    f"点云融合：已完成 {self.fusion_completed} 张；第 {i} 帧失败，请查看上方错误"
                ))
            else:
                message = self._fusion_message(scan, stamp)
            ...
            with self.fusion_lock:
                if result.returncode:
                    self.fusion_failed += 1
                else:
                    self.fusion_completed += 1
                self.fusion_pending = max(0, self.fusion_pending - 1)
                pending = self.fusion_pending
            if pending:
                self.after(0, lambda count=pending: self.fusion_status_info.set(
                    f"点云融合：已完成 {self.fusion_completed} 张，队列剩余 {count} 帧"
                ))
            elif result.returncode:
                self.after(0, lambda: self.fusion_status_info.set(
                    f"点云融合：已完成 {self.fusion_completed} 张，失败 {self.fusion_failed} 张"
                ))
    ...
    def _run_all_with_auto_capture_worker(self):
        try:
            ...
            self._close_auto_capture_views()
            self.after(0, lambda: self.status.set(
                "自动抓拍规则运动完成：图片已保存；后台点云融合将继续并逐帧显示 3D。"
            ))
        except Exception as error:
            self.auto_capture_failed = True
