    ...
    try:
        while rclpy.ok():
            ...
            try:
                ...
                pass
            # A disconnected/stalled SSH client must never leave the chassis
            # moving with its last command.
            if time.monotonic() - last_command_at > 0.5:
                linear = angular = 0.0
            message = Twist()
            ...
            if latest["pose"] is not None and now - latest["printed_at"] >= 0.08:
                x, y, yaw = latest["pose"]
                print(json.dumps({
                    "type": "pose", "x": x, "y": y, "yaw": yaw, "source": latest["source"],
                    "cmd_linear": linear, "cmd_angular": angular,
                    "cmd_age": max(0.0, now - last_command_at),
                }), flush=True)
